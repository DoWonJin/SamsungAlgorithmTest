...
int IsFalseCondition(int(*arry)[3], int object) {
	int m[3], n[3];
	int strike = 0;
	int ball = 0;
	divide(m, (*arry)[0]);
	...
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
		{
			if (m[i] == n[j] && i == j)strike++;
			else if (m[i] == n[j] && i != j)ball++;
		}
	//printf("%d %d\n\n", strike,ball);

	if ((*arry)[1] == strike && (*arry)[2] == ball)return 0;
	else return 1; // 조건과 다를 때
}
...
int main(void) {
	...
	for (int i = 123; i <= 987; i++) {
		...
		int dup = 1;
		for (int j = 0;j < num;j++)
			if (IsFalseCondition(arry + j, i)) {
				dup = 0;//조건에 맞지 않을 때
				break;
			}
		if (dup == 1) {//num개의 조건에 맞을 때
			//printf("%d가 조건에 맞음ㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁ\n", i);
			count++;
		}
	}
	printf("%d\n", count);
