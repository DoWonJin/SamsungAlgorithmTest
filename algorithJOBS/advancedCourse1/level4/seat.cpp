...
enum State { up = 0, right, down, left };

int arry[1000][1000] = { 0 };
...
int main() {
	...
	//stackOverflow발생하는데;; col을 1000으로 설정하면;;
	int order;
	enum State state;
	scanf("%d %d", &col, &row);
	scanf("%d", &order);
	...
	if (order > col*row)printf("0\n");
	else {
		for (int i = 0;i < row;i++) {
			for (int j = 0;j < col;j++) {
				if (arry[i][j] == order)printf("%d %d\n", j + 1, i + 1);
			}
		}
	}
